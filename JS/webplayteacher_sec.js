...
adapter.ParserXMPPString=function(IQMessage) {
    ...
    try {
        ...
        if (xmlnsVal === "jabber:iq:agents") {
            ...
            ROOM_JID_mcu = ROOM_JID + "@" + agentArr[1].getAttribute("jid");
            ROOM_JID_fs = ROOM_JID+"@"+agentArr[2].getAttribute("jid");
            adapter.imguploadurl = ServerHttp.replace(":80", ":18080/api");
            // adapter.imguploadurl = ServerHttp;
            adapter.connection.send($pres().tree());
        ...
        {
            adapter.pagejid=query.getElementsByTagName("jid")[0].firstChild.nodeValue;

            var titlename=adapter.pagejid.slice(adapter.pagejid.indexOf("/")+1);
            addwbpage(titlename,adapter.pagejid,800,600);
        }
                ...
                success:  function(data, textStatus) {
                    ...
                    $("#uploadFiles").click(function(event) {
                        ...
                        if(target.nodeName.toLowerCase()==="li"){
                            // var path = uploadList[target.innerHTML];
                            // console.log(path);
                            // path = path.substring(1);
                            // tmp1 = path.substring(0,path.indexOf("\\"));
                            // tmp2 = path.substring(path.indexOf("\\")+1);
                            // console.log(tmp1+"/"+tmp2);
                            // console.log(clientJid);
                            // console.log(filejid);
                            adapter.connection.send(sendCreatewbxml(filename));
                            // adapter.connection.send(achiveDownloadUrl(filejid,target.innerHTML,filesize));
                        }
                    });
                    //完成后调用回复xml的函数
                },
...
function achiveDownloadUrl(jid,text,filesize){
    console.log(jid);
...
//canvas 绑定mouseup
        $("#"+tit).children().eq(1).mouseup(function(e){
            ...
            switch(drawtype){
                ...
                case "thinfreeline":
                case "thickfreeline":ctx.closePath();adapter.connection.send(sendlinexml(adapter.pointArr,adapter.drawtype,adapter.pagejid,ctx.lineWidth));break;
